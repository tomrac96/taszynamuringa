class MaszynaTuringa:
    ...
    def zaladuj_maszyne_turinga(self, sciezka_pliku):
        ...
        for linia in linie:
            ...
            elif linia.startswith('! podzielnosc_przez'):
                self.podzielnosc = int(linia.split()[2])
            else:
                czesci = linia.split()
    ...
    def wyswietl_wynik(self):
        ...
        ostatni_stan = self.stan
        if ostatni_stan == 'koniec':
            liczba = ''.join(self.tasma)
            liczba = liczba.rstrip('_')  # Usunięcie '_' z końca liczby
            try:
                liczba_int = int(liczba)
                if liczba_int % self.podzielnosc == 0:
                    print(f'Werdykt: Tak, liczba jest podzielna przez {self.podzielnosc}.')
                else:
                    print(f'Werdykt: Nie, liczba nie jest podzielna przez {self.podzielnosc}.')
            except ValueError:
